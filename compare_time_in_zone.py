...
int_file = Path("data/traffic/video_int_time_stream.json")
float_file = Path("data/traffic/video_float_time_stream.json")
...
report_writer = csv.writer(
    open(
        str(int_file.with_suffix(".csv").absolute())
        .replace("_int", "")
        .replace("_time_stream", "_report"),
        "w",
        encoding="utf-8",
    )
)
report_writer.writerow(
    ["frame_number", "object_tracking_id", "int_time", "float_time", "difference"]
)
...
for idx, [int_frame, float_frame] in enumerate(zip(int_data, float_data)):
    ...
    for object_id in object_ids:
        int_time = int_frame.get(object_id, None)
        float_time = float_frame.get(object_id, None)

        if int_time != float_time:
        ...
        difference = round(float_time - int_time, 2)
        report_writer.writerow(
            [
                idx + 1,
                object_id,
